...
def test_rotation_reads_from_config_not_hardcoded():
    """Verify the sentinel reads language list from config, not from hardcoded."""
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        path = f.name
    try:
...
def test_rotation_cycle_all():
    """Full cycle through all 8 languages."""
    path = "/tmp/test_sentinel_cycle.json"
    try:
        for i in range(len(LANGS)):
            _write_config(path, i)
            new_lang, new_idx = advance_rotation(path)
            expected_next = (i + 1) % len(LANGS)
            assert new_idx == expected_next, f"At index {i}, expected {expected_next}, got {new_idx}"
    finally:
        os.unlink(path)