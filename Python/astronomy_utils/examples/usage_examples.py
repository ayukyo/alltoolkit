...
def example_planets():
    ...
    # 获取单个行星
    jupiter = mod.get_planet("木星")
    print(f"\n木星详情:")
    print(f"  卫星数: {jupiter.moons}")
    print(f"  距太阳: {mod.au_to_km(jupiter.distance_au):.1e} km")
...
def example_constellations():
    ...
    # 黄道十二宫
    zodiac = mod.get_zodiac_constellations()
    print(f"\n黄道十二宫:")
    for c in zodiac:
        print(f"  {c.name_cn} ({c.abbreviation}): {c.description}")
    
    # 获取特定星座
    leo = mod.get_constellation("狮子座")
    print(f"\n狮子座详情:")
    ...
    # 按月份查询
    june_const = mod.ConstellationData.get_constellation_by_month(6)
    print(f"\n6月最佳观测星座:")
    for c in june_const:
        print(f"  {c.name_cn}")
...
def example_stars():
    ...
    # 最亮的恒星（前5颗）
    print(f"\n最亮的恒星 (前5颗):")
    for name, data in list(mod.StarData.BRIGHT_STARS.items())[:5]:
    ...
    # 北京可见恒星
    visible = mod.StarData.get_visible_stars(39.9)  # 北京纬度
    print(f"\n北京可见恒星 (前10颗):")
...
def example_moon_phase():
    ...
    # 当前月相
    moon_age, phase_idx, name_cn, name_en = mod.calculate_moon_phase()
    print(f"\n当前月相:")
    ...
    print(f"\n特定日期月相:")
    for dt in dates:
        age, _, cn, en = mod.calculate_moon_phase(dt)
        print(f"  {dt.strftime('%Y-%m-%d')}: {cn} ({en}), 月龄 {age:.1f}天")
    
    # 下一个满月
    now = datetime.now()
    next_full = mod.MoonPhaseCalculator.calculate_next_moon_phase(now, 4)
    print(f"\n下一个满月: {next_full.strftime('%Y-%m-%d %H:%M')}")
    
    # 下一个新月
    next_new = mod.MoonPhaseCalculator.calculate_next_moon_phase(now, 0)
    print(f"下一个新月: {next_new.strftime('%Y-%m-%d %H:%M')}")
    
    # 月球距离
    distance = mod.MoonPhaseCalculator.calculate_moon_distance(now)
...
def example_coordinates():
    ...
    # 转换为地平坐标
    az, alt = mod.AstronomyCalculator.equatorial_to_horizontal(ra, dec, latitude, longitude, dt)
    print(f"\n天狼星在北京的地平坐标:")
    ...
    # 恒星时
    gst = mod.AstronomyCalculator.calculate_greenwich_sidereal_time(dt)
    lst = mod.AstronomyCalculator.calculate_local_sidereal_time(dt, longitude)
    print(f"\n恒星时:")
...
def example_rise_set():
    ...
    result = mod.AstronomyCalculator.calculate_rise_set_times(ra, dec, latitude, longitude, dt)
...
def example_distance():
    ...
    # 天文单位转千米
    earth_sun_km = mod.au_to_km(1.0)
    mars_sun_km = mod.au_to_km(1.524)
    print(f"\n行星距太阳:")
    print(f"  地球: {earth_sun_km:.1e} km = 1 AU")
    print(f"  火星: {mars_sun_km:.1e} km = 1.524 AU")
    
    # 光传播时间
    earth_sun_time = mod.calculate_light_travel_time(earth_sun_km)
    print(f"\n光传播时间:")
    print(f"  地球到太阳: {earth_sun_time.total_seconds():.0f} 秒 ({earth_sun_time.total_seconds()/60:.1f} 分钟)")
    
    # 月球到地球
    moon_distance = 384400
    moon_time = mod.calculate_light_travel_time(moon_distance)
    print(f"  月球到地球: {moon_time.total_seconds():.2f} 秒")
    
    # 角直径
    moon_angular = mod.AstronomyCalculator.calculate_angular_diameter(3474, 384400)
    sun_angular = mod.AstronomyCalculator.calculate_angular_diameter(696340, 149597870.7)
    print(f"\n角直径:")
    ...
    nearest_ly = mod.km_to_light_year(nearest_km)
    print(f"\n南门二:")
...
def example_full_demo():
    ...
    # 月相
    moon_age, _, name_cn, _ = mod.calculate_moon_phase(dt)
    ...
    # 恒星时（北京）
    gst = mod.AstronomyCalculator.calculate_greenwich_sidereal_time(dt)
    lst = mod.AstronomyCalculator.calculate_local_sidereal_time(dt, 116.4)
    print(f"格林尼治恒星时: {gst:.1f}°, 北京本地恒星时: {lst:.1f}°")
    
    # 可见恒星
    visible = mod.StarData.get_visible_stars(39.9)[:5]
    print(f"北京可见亮星: {', '.join([s['name_cn'] for s in visible])}")
    
    # 下一个满月
    next_full = mod.MoonPhaseCalculator.calculate_next_moon_phase(dt, 4)
